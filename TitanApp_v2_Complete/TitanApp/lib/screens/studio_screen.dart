...
class _StudioState extends State<StudioScreen> {
  ...
  void _applyCharacter(String name) {
    ...
    setState(() {
      _character = name;
      _preset = '';           // preset reset on character change
      _gender = c.gender;     // gender locked to character
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  SvgPicture.asset(e.value.icon, width: 28, height: 28, colorFilter: const ColorFilter.mode(Color(0xFF00FFFF), BlendMode.srcIn)),
                  const SizedBox(height: 4),
  ...
  Widget _buildAdvanced() {
    return MintCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _advToggle(
        ...
        Icons.air_rounded,
        _useBreaths,
        (v) => setState(() => _useBreaths = v),
      ),
      _advToggle(
        'Adaptive Pacing',
        '(Auto slows for long text 300+ chars)',
        Icons.speed_rounded,
        _adaptivePacing,
        (v) => setState(() => _adaptivePacing = v),
      ),
      _advToggle(
        'SSML Markup Support',
        ...
        Icons.code_rounded,
        _ssmlMode,
        (v) => setState(() => _ssmlMode = v),
      ),
      _advToggle(
        'Ultra-Low Latency Mode',
        '(Faster response, shorter texts)',
        Icons.bolt_rounded,
        _lowLatency,
        (v) => setState(() => _lowLatency = v),
      ),
      ...
      // ── HD / AI Features ──────────────────────────────
      Row(children: [
        const Icon(Icons.auto_awesome_rounded, size: 13, color: Color(0xFF00F2FE)),
        const SizedBox(width: 6),
        const Text('HD Neural Features',
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF00F2FE))),
      ]),
      const SizedBox(height: 8),

      _advToggle(
        'HD Neural Voice',
        '(Higher quality audio rendering)',
        Icons.hd_rounded,
        _hdVoice,
        (v) => setState(() => _hdVoice = v),
      ),
      _advToggle(
        'Auto Emotion Detection',
        '(AI detects sentiment & adjusts tone)',
        Icons.psychology_rounded,
        _autoEmotion,
        (v) => setState(() {
          _autoEmotion = v;
          if (v) _emotion = 'Normal'; // reset — AI will pick
        }),
      ),
      _advToggle(
        'Word-Boundary Sync',
        '(Precise word timing for karaoke/captions)',
        Icons.closed_caption_rounded,
        _wordBoundary,
        (v) => setState(() => _wordBoundary = v),
      ),
      _advToggle(
        'Avatar Mode (Beta)',
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ...[
              ...
              child: Text(t, style: const TextStyle(
                fontSize: 10, color: Color(0xFF5A6A7E),
