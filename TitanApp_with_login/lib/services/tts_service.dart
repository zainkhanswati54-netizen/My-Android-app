...
class TtsService {
  // ── Primary: Kaggle GPU (fast) ─────────────────────────
  // Kaggle notebook chalao, URL yahan paste karo
  static String _kaggleUrl = '';  // e.g. 'https://xxxx.ngrok.io'

  // ── Fallback: Hugging Face (slow but always on) ─────────
  static const String _hfUrl =
      'https://zainkhanswati54-titan-tts-api.hf.space/tts';

  // ── Kaggle URL set karo (Settings se) ──────────────────
  static void setKaggleUrl(String url) {
    _kaggleUrl = url.trim().replaceAll(RegExp(r'/tts\$'), '');
  }

  static String get _kaggleApi => _kaggleUrl.isNotEmpty ? '\$_kaggleUrl/tts' : '';
  static bool get hasKaggle => _kaggleUrl.isNotEmpty;

  // ── Smart URL — Kaggle pehle, HF fallback ──────────────
  static Future<String> _getWorkingUrl(Map<String, String> params) async {
    // Kaggle try karo agar set hai
    if (hasKaggle) {
      try {
        final uri = Uri.parse(_kaggleApi).replace(queryParameters: params);
      ...
      // Kaggle fail — HF fallback
    }
    return _hfUrl;
  }

  static String get _apiBase => hasKaggle ? _kaggleApi : _hfUrl;
