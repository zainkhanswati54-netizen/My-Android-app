...
class _AdminDashboardScreenState extends State<AdminDashboardScreen>
  ...
  final List<_AdminTab> _tabs = const [
    ...
    _AdminTab(icon: Icons.analytics_rounded,        label: 'Analytics'),
  ];
...
class _PinScreenState extends State<_PinScreen>
  ...
  Future<void> _checkPin() async {
    ...
    if (AdminService.verifyPin(_pin)) {
      ...
      AdminService.setAdminFlag(); // mark this user as admin for Firebase rules
      widget.onUnlocked();
...
class _CharactersTabState extends State<_CharactersTab> {
  ...
  Future<void> _toggle(String name, bool val) async {
    ...
    final success = await AdminService.setCharacterEnabled(name, val);
    if (mounted) {
      if (!success) {
        // Rollback on failure + show error
        setState(() => _states[name] = !val);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to update $name. Check Firebase rules.'),
  ...
  Future<void> _setAll(bool val) async {
    ...
    final success = await AdminService.setAllCharacters(val);
    await _load();
    if (mounted) {
      if (!success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to update characters. Check Firebase rules.'),
            backgroundColor: cRed,
...
class _MaintenanceTabState extends State<_MaintenanceTab> {
  ...
  Future<void> _save(bool enabled) async {
    setState(() => _saving = true);
    final success = await AdminService.setMaintenanceMode(
      enabled: enabled,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success
              ? (enabled ? '🔴 Maintenance mode ENABLED' : '🟢 App is now LIVE')
              : '❌ Save failed. Check Firebase rules.'),
          backgroundColor: success ? (enabled ? cRed : cGreen) : cRed,
          duration: const Duration(seconds: 3),
        ),
      );
    }
...
class _LimitsTabState extends State<_LimitsTab> {
  ...
  // Controllers — only Free (app is free, no premium)
  late TextEditingController _freeCharCtrl;
  late TextEditingController _freeDailyCtrl;
  late TextEditingController _freeHistCtrl;
  late TextEditingController _premiumCharCtrl;
  late TextEditingController _premiumDailyCtrl;
  late TextEditingController _premiumHistCtrl;
  ...
  void initState() {
    ...
    _premiumCharCtrl  = TextEditingController();
    _premiumDailyCtrl = TextEditingController();
    _premiumHistCtrl  = TextEditingController();
    _load();
    _refreshTimer = Timer.periodic(const Duration(seconds: 30), (_) => _load());
  }
  ...
  @override
  void dispose() {
    for (final c in [_freeCharCtrl, _freeDailyCtrl, _freeHistCtrl]) {
      c.dispose();
    }
    _refreshTimer?.cancel();
    super.dispose();
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _premiumCharCtrl.text  = d.premiumCharLimit.toString();
        _premiumDailyCtrl.text = d.premiumDailyLimit.toString();
        _premiumHistCtrl.text  = d.premiumHistoryLimit.toString();
        _loading = false;
      });
    }
  }
  ...
  Future<void> _save() async {
    ...
    final updated = AdminLimitsData(
      ...
      premiumCharLimit:    int.tryParse(_premiumCharCtrl.text)  ?? _data.premiumCharLimit,
      freeDailyLimit:      int.tryParse(_freeDailyCtrl.text)    ?? _data.freeDailyLimit,
      premiumDailyLimit:   int.tryParse(_premiumDailyCtrl.text) ?? _data.premiumDailyLimit,
      freeHistoryLimit:    int.tryParse(_freeHistCtrl.text)     ?? _data.freeHistoryLimit,
      premiumHistoryLimit: int.tryParse(_premiumHistCtrl.text)  ?? _data.premiumHistoryLimit,
    );
    final success = await AdminService.setLimits(updated);
    await _load();
    if (mounted) {
      setState(() => _saving = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? '✅ Limits saved!' : '❌ Save failed. Check Firebase rules.'),
          backgroundColor: success ? cGreen : cRed,
          duration: const Duration(seconds: 3),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    return _TabWrapper(
      ...
      subtitle: 'Chars/req: ${_data.freeCharLimit}  ·  Daily: ${_data.freeDailyLimit}',
      onRefresh: _load,
      saving: _saving,
      loading: _loading,
      child: Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            Icon(Icons.person_rounded, color: cBlue, size: 14),
            const SizedBox(width: 6),
            const Text('Free Users', style: TextStyle(color: cBlue, fontSize: 12, fontWeight: FontWeight.w700)),
          ]),
        ),
        ...
        Container(
          ...
          child: Row(children: [
            Icon(Icons.workspace_premium_rounded, color: cAmber, size: 14),
            const SizedBox(width: 6),
            const Text('Premium Users', style: TextStyle(color: cAmber, fontSize: 12, fontWeight: FontWeight.w700)),
          ]),
        ),
        const SizedBox(height: 10),
        _LimitRow(
          label: 'Chars Per Request',
          ctrl: _premiumCharCtrl,
          icon: Icons.text_fields_rounded,
          accent: cAmber,
        ),
        const SizedBox(height: 10),
