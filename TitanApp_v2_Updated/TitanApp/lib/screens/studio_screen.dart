...
class _StudioState extends State<StudioScreen> {
  ...
  bool _validateTextForLanguage(String text) {
    ...
    if (langCfg == null) return true;
    // Arabic script check (Urdu + Arabic)
    if (langCfg.script == LangScript.arabic) {
      final arabicRange = RegExp(r'[\u0600-\u06FF\u0750-\u077F\uFB50-\uFDFF\uFE70-\uFEFF]');
      return arabicRange.hasMatch(text);
    }
    // Devanagari check (Hindi)
    if (langCfg.script == LangScript.devanagari) {
      final devaRange = RegExp(r'[\u0900-\u097F]');
      return devaRange.hasMatch(text);
    }
    // Chinese check
    if (langCfg.script == LangScript.chinese) {
      final chineseRange = RegExp(r'[\u4E00-\u9FFF\u3400-\u4DBF]');
      return chineseRange.hasMatch(text);
    }
    // Latin (English, Spanish, French, German): accept anything
    return true;
  }

  String _getHintText() {
    return kLanguages[_language]?.hintText ?? 'Type your text here...';
  }
        ...
        Directionality(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: kLanguages[_language]?.hintText ?? 'Type your text here...',
