...
class TtsService {
  ...
  static Future<File> savePermanent(File temp, String filename) async {
    ...
    if (Platform.isAndroid) {
      final tryPaths = [
        '/storage/emulated/0/Download/TitanStudioPRO',
        '/storage/emulated/0/Downloads/TitanStudioPRO',
        '/storage/emulated/0/Music/TitanStudioPRO',
      ];
      for (final dirPath in tryPaths) {
        try {
          final dir = Directory(dirPath);
          await dir.create(recursive: true);
          final f = File('$dirPath/$finalName');
    ...
    if (dest == null || !(await dest.exists()) || (await dest.length()) < 500) {
      ...
      final fallbackDir = Directory('${appDir.path}/TitanStudioPRO');
