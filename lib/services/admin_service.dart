...
class AdminService {
  ...
  // ── Mark current user as admin in Firebase (for tts_logs read access) ──
  static Future<void> setAdminFlag() async {
    try {
      final uid = FirebaseAuth.instance.currentUser?.uid;
      if (uid == null) return;
      final token = await FirebaseAuth.instance.currentUser?.getIdToken();
      final uri = token != null
          ? Uri.parse('\$_dbUrl/admin/admins/\$uid.json?auth=\$token')
          : Uri.parse('\$_dbUrl/admin/admins/\$uid.json');
      await http.put(uri,
          headers: {'Content-Type': 'application/json'},
          body: 'true').timeout(const Duration(seconds: 8));
    } catch (_) {}
  }
