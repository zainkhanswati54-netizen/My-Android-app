...
class _StudioState extends State<StudioScreen>
  ...
  Future<void> _generate() async {
    ...
    if (result.ok) {
      ...
      AnalyticsService.logGeneration(
        text:           text,
        character:      _character,
        language:       _language,
        emotion:        _emotion,
        ...
        pitchVal:       _pitch.round(),
        success:        true,
        durationMs:     DateTime.now().millisecondsSinceEpoch - genStart,
        useBreaths:     _useBreaths,
        adaptivePacing: _adaptivePacing,
        ssmlMode:       _ssmlMode,
      );
      // ── Show interstitial ad (based on frequency setting) ──
      AdService.onGenerationComplete();
    } else {
      setState(() {
        _generating = false;
        _progress = 0;
  ...
  Widget _buildCharacters() => MintCard(
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    SizedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(e.key, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                            color: disabled ? cMuted : (selected ? Colors.white : cText)),
                            textAlign: TextAlign.center),
                          if (cfg.specialtyLabel.isNotEmpty)
                            Text(cfg.specialtyLabel, style: TextStyle(fontSize: 10,
      ...
      if (_character.isNotEmpty) ...[
        const SizedBox(height: 8),
        Builder(builder: (sfCtx) {
          final isDkSF = Theme.of(sfCtx).brightness == Brightness.dark;
                ...
                child: Stack(
                  children: [
                    Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Center(child: SvgPicture.asset(e.value.icon, width: 24, height: 24, colorFilter: ColorFilter.mode(presetIconCol, BlendMode.srcIn))),
                      const SizedBox(height: 4),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: Icon(Icons.search_rounded, size: 16, color: mutedColor),
                ...
                children: filtered.map((lang) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: sel ? accentColor : bgCard,
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(
                            color: sel ? accentColor : borderCol.withOpacity(0.6)),
                      ),
                      child: Row(children: [
                        SvgPicture.asset(cfg.flag, width: 20, height: 20),
                        const SizedBox(width: 8),
                        Text(lang, style: TextStyle(fontSize: 13,
                            fontWeight: FontWeight.w700,
                            color: sel ? Colors.white : textColor)),
                        const Spacer(),
                        Text(cfg.label, style: TextStyle(fontSize: 10,
                            fontWeight: FontWeight.w800,
                            color: sel ? Colors.white70 : mutedColor)),
                        if (sel) ...[
                          const SizedBox(width: 4),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(subtitle, style: const TextStyle(
            fontSize: 11, color: Color(0xFF5A6A7E), height: 1.3)),
          if (lockedMsg != null) ...[
            const SizedBox(height: 4),
            Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 11,
              color: _textCtrl.text.length >= _charLimit ? cRed : tiCountCol,
          ...
          Builder(builder: (xCtx) {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: xRed.withOpacity(0.1), borderRadius: BorderRadius.circular(6)),
              child: Icon(Icons.clear_rounded, color: xRed, size: 16),
            ),
          );
          }),
        ]),
        ...
        Directionality(
          ...
          child: TextField(
            ...
            style: TextStyle(fontSize: 15, color: tiTextCol, height: 1.5),
            textDirection: _getTextDir(),
            textAlign: langCfg.isRtl ? TextAlign.right : TextAlign.left,
            decoration: InputDecoration(
              hintText: _getHintText(),
              hintStyle: TextStyle(color: tiHintCol, fontSize: 14),
              ...
              enabledBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
                borderSide: BorderSide(color: _textCtrl.text.length >= _charLimit ? cRed.withOpacity(0.5) : tiBorderCol),
              ),
              filled: true, fillColor: tiFillCol,
        ...
        Builder(builder: (btnCtx) {
          ...
          Expanded(child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: clearCol.withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: clearCol.withOpacity(0.35))),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.refresh_rounded, size: 15, color: clearCol),
                const SizedBox(width: 5),
                Text('Clear', style: TextStyle(fontSize: 12, color: clearCol, fontWeight: FontWeight.w700)),
              ]),
            ),
          )),
          const SizedBox(width: 8),
          Expanded(child: GestureDetector(
            ...
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 9),
              decoration: BoxDecoration(
                color: pasteCol.withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: pasteCol.withOpacity(0.35))),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.content_paste_rounded, size: 15, color: pasteCol),
                const SizedBox(width: 5),
                Text('Paste', style: TextStyle(fontSize: 12, color: pasteCol, fontWeight: FontWeight.w700)),
              ]),
            ),
          )),
        ]);
    ...
    Expanded(child: GestureDetector(
      ...
      child: Container(
        height: 50, decoration: BoxDecoration(
          color: navAccent.withOpacity(0.1),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: navAccent.withOpacity(0.35))),
        child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.history_rounded, color: navAccent, size: 20),
          const SizedBox(width: 8),
          Text('Download History', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: navAccent)),
        ])),
      ),
    )),
  ]);
