...
class _GoogleLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Blue: from -35° spanning to about 80° (top, going right)
    canvas.drawArc(rect, -35 * pi / 180, 80 * pi / 180, false, p(const Color(0xFF4285F4)));
    // Red: from ~-120° spanning ~85° (top left)
    canvas.drawArc(rect, -120 * pi / 180, 85 * pi / 180, false, p(const Color(0xFFEA4335)));
    // Yellow: from ~160° spanning ~90° (bottom left to bottom)
    canvas.drawArc(rect, 160 * pi / 180, 85 * pi / 180, false, p(const Color(0xFFFBBC05)));
    // Green: from ~45° spanning ~115° (bottom right)
    canvas.drawArc(rect, 45 * pi / 180, 115 * pi / 180, false, p(const Color(0xFF34A853)));

    // White inner circle to create hollow effect
    canvas.drawCircle(Offset(cx, cy), r - sw / 2 - 0.5, Paint()..color = const Color(0xFF1A2E21));

    // Blue horizontal bar (the crossbar of G)
    final barLeft  = cx;
    final barRight = cx + r + sw / 2;
    final barTop   = cy - sw / 2;
    final barBot   = cy + sw / 2;
    canvas.drawRect(
      Rect.fromLTRB(barLeft, barTop, barRight, barBot),
      Paint()..color = const Color(0xFF4285F4),
    );
    // Blue fill in right half of circle to complete G shape
    canvas.drawRect(
      Rect.fromLTRB(cx, cy - sw * 1.1, cx + r + sw / 2, cy + sw / 2),
      Paint()..color = const Color(0xFF1A2E21),
    );
    canvas.drawRect(
      Rect.fromLTRB(cx, barTop, cx + r + sw / 2, barBot),
      Paint()..color = const Color(0xFF4285F4),
    );
  }
