...
class AdminService {
  ...
  static const String _adminPin = '1234'; // ← Apna PIN yahan set karo

  // ── PIN Verify ───────────────────────────────────────
  static bool verifyPin(String pin) => pin == _adminPin;

  // ══════════════════════════════════════════════════════
  //  HELPERS: Firebase REST API (no Firestore package needed)
  // ══════════════════════════════════════════════════════
  ...
  static Future<Map<String, dynamic>?> _get(String path) async {
    try {
      ...
      final auth = token != null ? '?auth=$token' : '';
      final res = await http
          .get(Uri.parse('$_dbUrl/$path.json$auth'))
  ...
  static Future<bool> _set(String path, dynamic data) async {
    try {
      ...
      final res = await http
          .put(
            Uri.parse('$_dbUrl/$path.json$auth'),
  ...
  // ══════════════════════════════════════════════════════
  //  3. CHARACTER LIMIT (Free vs Premium)
  // ══════════════════════════════════════════════════════
  ...
  static Future<List<AdminUserRecord>> getUserAudit() async {
    try {
      // Read from admin/users node (app writes here on login)
      final token = await _getToken();
      final auth = token != null ? '?auth=$token' : '';
      final res = await http
          .get(Uri.parse('$_dbUrl/admin/users.json$auth'))
  ...
  // ── Get abuse/request logs ────────────────────────────
  static Future<List<AdminRequestLog>> getRequestLogs() async {
    try {
      final token = await _getToken();
      final auth = token != null ? '?auth=$token' : '';
      // Get last 50 logs ordered by timestamp
      final res = await http
          .get(Uri.parse(
              '$_dbUrl/admin/request_logs.json$auth&orderBy="\$key"&limitToLast=50'))
          .timeout(const Duration(seconds: 15));
      if (res.statusCode != 200 || res.body == 'null') return [];
      final raw = jsonDecode(res.body);
      if (raw is! Map) return [];
...
class AdminLimitsData {
  final int freeCharLimit;     // Free user per-request char limit
  final int premiumCharLimit;  // Premium user per-request char limit
  final int freeDailyLimit;    // Free user daily requests
  final int premiumDailyLimit; // Premium user daily requests
  final int freeHistoryLimit;  // Free user history entries
...
class AdminPremiumData {
  ...
  final bool premiumCharactersLocked; // Titan, Seraph, Atlas, etc.
...
class AdminAdData {
  ...
  final int interstitialFrequency; // show every N TTS generations
  final bool testMode;
